...
function init(){
	...
	brick = [];
	item_array = [];
	$("#interface").show();// 인터페이스 화면 띄우기 -김영록 수정- 5/28
		...
		$(document).on("keydown",function(e){
			if(e.key == " "){ // 스페이스바 대신 q값으로 바꿈 김영록(05/20)
				...
				if(life <= 0){
					...
					$("#item").text("아이템 : " + item_count + "/" + item_array.length/3);
					$("#container").css({
						"background-image": "url('brick_dark.jpg')"
					}); // 게임 배경색 초기화
				}
